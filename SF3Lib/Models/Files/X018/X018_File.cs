...
namespace SF3.Models.Files.X018 {
    public class X018_File : ScenarioTableFile, IX018_File {
        public override int RamAddress => 0x06088000;
        public override int RamAddressLimit => 0x0609D800;

        protected X018_File(IByteData data, INameGetterContext nameContext, ScenarioType scenario) : base(data, nameContext, scenario) {
        ...
        public static X018_File Create(IByteData data, INameGetterContext nameContext, ScenarioType scenario) {
            var newFile = new X018_File(data, nameContext, scenario);
            if (!newFile.Init())
