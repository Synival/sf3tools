...
namespace SF3.Models.Tables.MPD.Model {
    public class ModelInstanceTable : FixedSizeTable<Structs.MPD.Model.ModelInstance> {
        protected ModelInstanceTable(IByteData data, MPD_CollectionType collection, string name, int address, int count, bool hasTagsAndFlags)
        : base(data, name, address, count) {
            Collection      = collection;
            HasTagsAndFlags = hasTagsAndFlags;
        }

        public static ModelInstanceTable Create(IByteData data, MPD_CollectionType collection, string name, int address, int count, bool hasTagsAndFlags)
