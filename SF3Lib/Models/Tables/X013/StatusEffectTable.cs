...
namespace SF3.Models.Tables.X013 {
    public class StatusEffectTable : ResourceTable<StatusEffect> {
        protected StatusEffectTable(IByteData data, string name, string resourceFile, int address) : base(data, name, resourceFile, address, 1000) {
        }

        public static StatusEffectTable Create(IByteData data, string name, string resourceFile, int address) {
            var newTable = new StatusEffectTable(data, name, resourceFile, address);
            if (!newTable.Load())
