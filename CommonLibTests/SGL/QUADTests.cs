...
/*
        [TestMethod]
        public void GetAbnormalFromNormals_With000SlopeNormals_ReturnsExpectedAbnormal() {
            var normal000Slope = new CompressedFixedVector(0, 1, 0);
            var normals = new CompressedFixedVector[] {
                normal000Slope, normal000Slope, normal000Slope, normal000Slope };

            var abnormal = normals.GetAbnormalFromNormals();

            AreVectorsEqual(new CompressedFixedVector(0.0000000f, 0.0000305f, 0.0000000f), abnormal, 0.00001f);
        }

        [TestMethod]
        public void GetAbnormalFromNormals_With100SlopeNormals_ReturnsExpectedAbnormal() {
            var normal100Slope = new CompressedFixedVector(0, 0.70710678118f, 0.70710678118f);

            var normals = new CompressedFixedVector[] {
                normal100Slope, normal100Slope, normal100Slope, normal100Slope
            };

            var abnormal = normals.GetAbnormalFromNormals();

             AreVectorsEqual(new CompressedFixedVector(0.0000000f, 0.2111511f, -0.8943786f), abnormal, c_minDelta);
        }

        [TestMethod]
        public void GetAbnormalFromNormals_With050SlopeAnd000SlopeNormals_ReturnsExpectedAbnormal() {
            var normal000Slope = new CompressedFixedVector(0, 1, 0);
            var normal050Slope = new CompressedFixedVector(0, 0.89442719099f, 0.44721359551f);

            float slopedWeight = 1.0842f;
            var normals = new CompressedFixedVector[] {
                normal000Slope, normal050Slope * slopedWeight
            };

            var abnormal = normals.GetAbnormalFromNormals();

            AreVectorsEqual(new CompressedFixedVector(0, 0.014984131f, -0.24435425f), abnormal, c_minDelta);
        }

        [TestMethod]
        public void GetAbnormalFromNormals_With100SlopeAnd000SlopeNormals_ReturnsExpectedAbnormal() {
            var normal000Slope = new CompressedFixedVector(0, 1, 0);
            var normal100Slope = new CompressedFixedVector(0, 0.70710678118f, 0.70710678118f);

            float slopedWeight = 1.267f;
            var normals = new CompressedFixedVector[] {
                normal000Slope, normal100Slope * slopedWeight
            };

            var abnormal = normals.GetAbnormalFromNormals();

            AreVectorsEqual(new CompressedFixedVector(0.0000000f, 0.05355835f, -0.4597266f), abnormal, c_minDelta);
        }

        [TestMethod]
        public void GetAbnormalFromNormals_With200SlopeAnd000SlopeNormals_ReturnsExpectedAbnormal() {
            var normal000Slope = new CompressedFixedVector(0, 1, 0);
            var normal200Slope = new CompressedFixedVector(0, 0.4472137f, 0.8944271f);

            float slopedWeight = 1.580f;
            var normals = new CompressedFixedVector[] {
                normal000Slope, normal200Slope * slopedWeight
            };

            var abnormal = normals.GetAbnormalFromNormals();

            AreVectorsEqual(new CompressedFixedVector(0.0000000f, 0.1522522f, -0.7653198f), abnormal, c_minDelta);
        }
*/
    }
}
