        ...
        public void CompressDecompress_ConsistencyCheck_Chunk5() {
            foreach (var st in Enum.GetValues<ScenarioType>()) {
                ...
                TestCase.Run(testCases, testCase => {
                    using (var mpdFile = MPD_Editor.Create(
                        new ByteEditor(File.ReadAllBytes(testCase.Filename)), nameGetterContext, st)) {
                        var data = mpdFile.CompressedEditors[5]?.Data;
                        if (data == null)
                            return;
            ...
            foreach (var st in Enum.GetValues<ScenarioType>()) {
                ...
                TestCase.Run(testCases, testCase => {
                    using (var mpdFile = MPD_Editor.Create(
                        ...
                        for (int i = 6; i <= 10; i++) {
                            var data = mpdFile.CompressedEditors[i]?.Data;
